            ...
            if len(ent) > 1:
                with doc.retokenize() as retokenizer:
                    ...
                    retokenizer.merge(np,
                                      attrs={LEMMA: doc.vocab.strings['_'.join([ele.lemma_.lower() for ele in np])]})
...
def spacy_np_no_stop(text):
    ...
    tokenized_doc = [ele.text.lower().strip().replace(' ', '_') for ele in doc if
                     ((not ele.is_stop) and (not ele.is_space) and (not ele.is_punct))]
    tokenized_doc = [ele for ele in tokenized_doc if ele]
    return tokenized_doc
...
def spacy_np_lemma_no_stop(text):
    ...
    tokenized_doc = [ele.lemma_.lower().replace(' ', '_') for ele in doc if
                     ((not ele.is_stop) and (not ele.is_space) and (not ele.is_punct))]
    tokenized_doc = [ele for ele in tokenized_doc if ele]
